...
def rec(direction='LONG', score=65, ret=2.0, raw=2.0, reason='SCORE_BELOW_SIGNAL_THRESHOLD', obstacle=0, symbol='BTCUSDT'):
    return {
        ...
        'horizons':{
            '4h':{'directional_return_pct':ret,'change_pct':raw},
            '8h':{'directional_return_pct':ret,'change_pct':raw},
...
def test_directional_gain_is_missed_opportunity():
    assert w.classify(rec(ret=1.5),12) == 'MISSED_DIRECTIONAL_OPPORTUNITY'


def test_directional_loss_is_correct_wait_protection():
    assert w.classify(rec(ret=-1.5, raw=-1.5),12) == 'WAIT_PROTECTED_CAPITAL'
...
def test_no_consensus_big_move_is_not_called_missed_signal():
    ...
    assert w.classify(r,12) == 'MATERIAL_MOVE_WITHOUT_CONSENSUS'


def test_obstacle_penalty_maps_to_canonical_structure_blocker():
    assert w.blocker(rec(obstacle=-8)) == 'NO_STRUCTURE'


def test_structure_segment_tracks_severity_and_score_gap():
    assert w.blocker_segment(rec(score=67, obstacle=-8)) == 'NO_STRUCTURE|PENALTY_8_PLUS|GAP_0_1'
    assert w.blocker_segment(rec(score=65, obstacle=-4)) == 'NO_STRUCTURE|PENALTY_4_7|GAP_2_3'


def test_diagnostics_never_change_threshold_or_execution():
    out=w.diagnose({'records':[rec(),rec(ret=-2,raw=-2)]},12)
    ...
    assert out['evaluation_horizons_h'] == [4,8,12]
...
def test_small_sample_cannot_propose_shadow_adjustment():
    ...
    p=next(x for x in c['proposals'] if x['blocker']=='NO_STRUCTURE')
...
def test_protective_broad_blocker_can_contain_eligible_narrow_segment():
    ...
    c=w.calibration({'records':rows})
    broad=next(x for x in c['proposals'] if x['blocker']=='NO_STRUCTURE')
    narrow=next(x for x in c['segment_proposals'] if x['segment']=='NO_STRUCTURE|PENALTY_8_PLUS|GAP_0_1')
...
def test_no_directional_consensus_is_never_auto_relaxed():
    ...
    c=w.calibration({'records':rows})
    p=next(x for x in c['proposals'] if x['blocker']=='NO_CONSENSUS')
    assert p['eligible_for_shadow_experiment'] is False
    assert p['suggested_shadow_adjustment_points'] == 0
...
if __name__ == '__main__':
    ...
    test_obstacle_penalty_maps_to_canonical_structure_blocker()
