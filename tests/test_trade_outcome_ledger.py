...
def test_signal_scope_excludes_research_only_rows():
    rows = [row(id='signal'), row(id='research', champion_take=False, research_sampling_lane=True)]
    signals = ledger.build_ledger(rows, horizon=24, scope='signals')
    all_rows = ledger.build_ledger(rows, horizon=24, scope='all')
    assert [x['id'] for x in signals] == ['signal']
...
if __name__ == '__main__':
    ...
    test_signal_scope_excludes_research_only_rows()
