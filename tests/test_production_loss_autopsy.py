...
def test_tight_stop_candidate_detected_without_mutating_policy():
 ...
 assert r['research_recommendation']['action']=='COLLECT_POST_FIX_COHORT'
...
def test_geometry_generations_are_separated():
 rows=[row(stop=99.8,version='LEGACY'),row(outcome='WIN_TP2',r=2,stop=98.5,version='DECISION_ENGINE_V8_CONTINUATION_AWARE')]
 r=a.build_report(rows)
 cohorts=r['entry_quality_evidence']['geometry_generations']
 assert cohorts['LEGACY']['tight_stop_losses']==1
 assert cohorts['DECISION_ENGINE_V8_CONTINUATION_AWARE']['wins']==1
 assert r['entry_quality_evidence']['generation_metadata_complete'] is True
 assert r['research_recommendation']['action']=='COMPARE_GEOMETRY_GENERATIONS'

if __name__=='__main__':
 test_post_tp1_is_separate_failure_mode(); test_tight_stop_candidate_detected_without_mutating_policy(); test_pre_tp1_failure_not_blindly_called_stop_problem(); test_geometry_generations_are_separated(); print('production loss autopsy tests: ok')
