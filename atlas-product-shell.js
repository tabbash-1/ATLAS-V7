(() => {
  ...
  function renderAi(ai,prod){
    ...
    $('apsAiTactical').textContent=tac?.direction?`${simpleDir(tac.direction)} · 1–3H`:'No clear short-term setup';
    $('apsAiBest').textContent=simpleAction(best,ai,prod);
    $('apsAiGeometry').textContent=best.entry==null?'No trade levels yet':`Entry ${fmt(best.entry)} · Stop ${fmt(best.stop_loss)} · Target ${fmt(best.target)} · R:R ${fmt(best.risk_reward,2)}`;
    ...
    $('apsAiTrigger').textContent=trigger||'Wait for confirmation before entering.';
  ...
  function productionWhy(p){
    ...
    if(p.production_signal_qualified&&p.execution_ready)return `Production signal qualified and execution geometry passed.${candidate}`;
    if(p.production_signal_qualified&&!p.execution_ready)return `Production score qualified at ${fmt(p.score,0)}/${fmt(p.signal_threshold,0)}, but execution is blocked by ${humanize(p.actionable_reason||p.geometry_gate?.reason||'geometry')}.${candidate}`;
  ...
  function productionRisk(p){
    ...
    const bits=[];
    if(p.geometry_gate?.status==='BLOCK')bits.push(`Geometry blocked: ${humanize(p.geometry_gate.reason)}`);
    if(Number.isFinite(Number(p.risk_reward)))bits.push(`R:R ${fmt(p.risk_reward,2)}`);
    ...
    return bits.length?bits.join(' · '):'No additional Production risk block.';
  }
  function productionChange(p){
    if(!p)return 'Wait for stronger direction, structure and risk/reward confirmation.';
    if(p.production_signal_qualified&&!p.execution_ready)return `Execution needs valid Entry/SL/TP geometry and R:R of at least ${fmt(p.geometry_gate?.min_risk_reward??1,1)}.`;
