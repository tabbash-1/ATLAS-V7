...
Consumes the canonical 4-12H forward evaluation report and emits diagnostic cautions.
...
SCHEMA='ATLAS_FORWARD_ROBUSTNESS_GUARDRAILS_V2_CANONICAL_TRUTH'
MIN_N=20
...
def fold_4h_positive(report,side):
    folds=report.get('trade_ready_chronological_folds') or []
    vals=[]
    for fold in folds:
...
def build(r):
    flags=[]
    reg=r.get('trade_ready_by_regime') or {}; pb=r.get('trade_ready_by_playbook') or {}; dirs=r.get('trade_ready_by_direction') or {}
    ...
    return {'schema':SCHEMA,'generated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'research_only':True,'shadow_only':True,'live_execution':False,'can_override_production':False,'can_change_threshold':False,'decision_source_of_truth':'FINAL_TRADE_GATE','product_horizon':'4-12H','source_schema':r.get('schema'),'source_generated_at':r.get('generated_at'),'minimum_sample':MIN_N,'flags':flags,'flag_count':len(flags),'promotion_policy':'NO_PRODUCTION_CHANGE_WITHOUT_NEW_PROSPECTIVE_VALIDATION'}
