"""Cached-only API for committed ATLAS research reports.
...
VERSION='ATLAS_COMMITTED_RESEARCH_API_V1'
REPORTS={
    '/api/research/offline-forward-evaluation':('offline-forward-evaluation-latest.json','ATLAS_OFFLINE_FORWARD_EVALUATION_V3_ROBUSTNESS'),
    '/api/research/forward-robustness-guardrails':('forward-robustness-guardrails-latest.json','ATLAS_FORWARD_ROBUSTNESS_GUARDRAILS_V1_SHADOW_ONLY'),
    '/api/research/prospective-direction-guardrail':('prospective-direction-guardrail-latest.json','ATLAS_PROSPECTIVE_DIRECTION_GUARDRAIL_V1_4H'),
}
...
def _load(base:Path,filename:str,schema:str):
    path=base/'status'/filename
    try:
        raw=json.loads(path.read_text(encoding='utf-8'))
        if raw.get('schema')!=schema:raise ValueError(f'schema mismatch: {raw.get("schema")}')
        if raw.get('research_only') is not True or raw.get('live_execution') is not False:raise ValueError('research safety contract mismatch')
        ...
        return {'ok':False,'schema':schema,'research_only':True,'live_execution':False,'can_override_production':False,'cached_only':True,'served_from':'COMMITTED_GITHUB_ACTIONS_REPORT','web_process_refresh_triggered':False,'request_time_outcome_read':False,'request_time_market_fetch':False,'web_process_background_worker':False,'api_version':VERSION,'error':f'{type(exc).__name__}: {exc}'}
