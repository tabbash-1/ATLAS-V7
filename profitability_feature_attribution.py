...
def value(e,k):
 c=e.get('context') or {}
 aliases={'score':['score'],'rsi':['rsi14','rsi'],'atr':['atr14','atr'],'momentum':['momentum_24h_pct','momentum'],'paced_relative_volume':['paced_relative_volume','relative_volume'],'relative_strength_adjustment':['relative_strength_adjustment'],'futures_adjustment':['futures_adjustment'],'obstacle_adjustment':['obstacle_adjustment'],'extension_adjustment':['extension_guard_adjustment','extension_adjustment'],'body_atr':['current_body_atr','body_atr']}
...
def build():
 ...
 out={}
 for f in FEATURES:
  clean=sorted(x for x in (value(e,f) for e,_ in disc) if x is not None)
