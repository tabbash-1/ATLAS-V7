...
def build(root: Path):
    ...
    all_terminal=[x for x in src.get("rows") or [] if (x.get("settlement") or {}).get("terminal") is True]
    ...
    return {
        ...
        "coverage":{"terminal_post_v2":len(all_terminal),"frozen_provenance_terminal":len(rows),
                    "path_only_terminal":len(all_terminal)-len(rows),
                    "coverage_pct":round(100*len(rows)/len(all_terminal),2) if all_terminal else None,
