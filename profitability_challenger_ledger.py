...
def registry_payload(reg):
 return {'frozen_at':reg.get('frozen_at'),'challengers':reg.get('challengers') or []}
def fingerprint(reg):
 raw=json.dumps(registry_payload(reg),sort_keys=True,separators=(',',':')).encode()
 return hashlib.sha256(raw).hexdigest()
def validate_registry(reg):
 if parse_ts(reg.get('frozen_at')) is None: raise RuntimeError('invalid challenger registry frozen_at')
 expected=reg.get('integrity_sha256')
 if expected and expected!=fingerprint(reg): raise RuntimeError('challenger registry integrity mismatch')
 return reg
...
def load_or_freeze():
 if REGISTRY.exists(): return validate_registry(json.loads(REGISTRY.read_text()))
 r=json.loads(RESEARCH.read_text()); now=datetime.now(timezone.utc).isoformat(); reg={'schema':'ATLAS_PROFITABILITY_CHALLENGER_REGISTRY_V2_INTEGRITY','frozen_at':now,'source_schema':r.get('schema'),'challengers':[{'challenger_id':x['challenger_id'],'frozen_filter':x['frozen_filter']} for x in r.get('challenger_registry') or []],'immutable_policy':'Never rewrite filters automatically; observations must be captured strictly after frozen_at.','production_effect':'NONE'}; reg['integrity_sha256']=fingerprint(reg); REGISTRY.parent.mkdir(parents=True,exist_ok=True); REGISTRY.write_text(json.dumps(reg,indent=2,sort_keys=True)); return reg
...
def build():
 reg=load_or_freeze(); attr=json.loads(ATTR.read_text()); freeze=parse_ts(reg['frozen_at']); forward=json.loads(FORWARD.read_text()) if FORWARD.exists() else {'trades':[]}; by_id={x.get('id'):x for x in forward.get('trades') or []}; rows=[]
 for e in attr.get('entries') or []:
  s=e.get('settlement') or {}; r=num(s.get('r_multiple')); captured=parse_ts(e.get('captured_at'))
  if captured is not None and captured>freeze and s.get('terminal') and r is not None:
 ...
 return {'schema':'ATLAS_PROFITABILITY_PROSPECTIVE_SHADOW_V3_INTEGRITY_COST_AWARE','frozen_at':reg['frozen_at'],'registry_integrity_sha256':reg.get('integrity_sha256') or fingerprint(reg),'new_terminal_market_observations':len(rows),'challengers':challengers,'promotion_policy':{'min_n':MIN_N,'execution_cost_bps_roundtrip':EXECUTION_COST_BPS,'max_drawdown_r':MAX_DD_R,'requires_cost_adjusted_positive_expectancy':True,'requires_funding_when_nonspot':True,'automatic_promotion':False},'cost_method':'execution_cost_R = roundtrip_bps/10000 * paper_notional_usd/risk_usd; spot funding=0; non-spot funding must be observed, never assumed.','stage':'PROSPECTIVE_SHADOW','production_mutation_authorized':False,'research_only':True}
def main():
