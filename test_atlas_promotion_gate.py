...
def rows(n=30,val=.2,d='LONG',offset=0):
    return [{'captured_at':f'2026-09-{1+(i+offset)//24:02d}T{(i+offset)%24:02d}:00:00Z','shadow_action':d,'net_12h_return_pct':val} for i in range(n)]
...
def test_late_segment_failure_rejected():
    r=rows(15,.3,offset=0)+rows(15,-.2,offset=15)
    e=p.evaluate('x',m(),r,.1)
    assert e['chronological_split']['early']['net_expectancy_pct']>0
    assert e['chronological_split']['late']['net_expectancy_pct']<0
