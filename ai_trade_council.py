...
The verified Production trade_plan is canonical. AI may explain or compare
alternatives, but it may not replace an ACTIONABLE Production plan with WAIT,
pullback, or breakout advice.
"""
...
def _evidence(decision):
    ...
    room=_num(tac.get('usable_room_pct') or tac.get('room_to_obstacle_pct')); ema20=_num(ind.get('ema20')); ema50=_num(ind.get('ema50')); px=_num(decision.get('entry'))
    pieces=[]
...
def _breakout_anchor(decision,direction,px):
    geom=decision.get('structural_geometry') or {}; br=geom.get('breakout') or {}; obstacle=_num(geom.get('obstacle_price'))
...
def _counterfactuals(decision,direction):
    ...
    s=1 if direction=='LONG' else -1; atr=atr or px*.01
    tac=decision.get('tactical_opportunity') or {}; target=_num(tac.get('target')); stop=_num(tac.get('stop_loss'))
    def add(name,entry,sl,tp,trigger):
        risk=abs(entry-sl); reward=abs(tp-entry); scenarios.append({'scenario':name,'direction':direction,'entry':round(entry,10),'stop_loss':round(sl,10),'target':round(tp,10),'risk_reward':round(reward/risk,3) if risk else None,'trigger':trigger,'shadow_only':True,'canonical':False})
    add('ENTER_NOW_SHADOW',px,stop or px-s*atr*.65,target or px+s*atr*1.2,'Shadow comparison only; cannot replace canonical Production plan.')
    level,source=_breakout_anchor(decision,direction,px)
    if level is not None:
        buffer=max(atr*.10,abs(level)*.0005); brk=level+s*buffer; add('WAIT_BREAKOUT_SHADOW',brk,brk-s*atr*.75,brk+s*atr*1.6,f'1H close/hold beyond {level:.8g} ({source})')
    return scenarios
...
def analyze(decision):
    ...
    tac=decision.get('tactical_opportunity') or {}; rr=_num(tac.get('risk_reward')); prod_dir=decision.get('candidate_direction'); prod_ok=bool(decision.get('production_signal_qualified') or decision.get('signal_qualified')); execution=bool(decision.get('execution_ready'))
    agree=(prod_dir==direction) if prod_dir else False
    if execution and prod_ok:
        verdict='CONFIRM_PRODUCTION' if agree else 'PRODUCTION_PRIORITY'; reason='CANONICAL_PRODUCTION_ACTIONABLE'; hybrid='CONFIRM'
    ...
    confidence=round(max(50,min(92,50+strength*45))); scenarios=_counterfactuals(decision,direction); canonical=_canonical_action(decision)
    if canonical.get('status')=='ACTIONABLE': best=next((x for x in scenarios if x.get('scenario')=='PRODUCTION_NOW'),None)
        ...
        viable=[x for x in scenarios if x.get('risk_reward') is not None]; viable.sort(key=lambda x:x.get('risk_reward') or 0,reverse=True); best=viable[0] if viable else None
