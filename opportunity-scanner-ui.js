...
function paintTile(id,text){
 ...
 const tone=/ENTER LONG|ENTER SHORT/.test(upper)?'positive':/UNAVAILABLE|STALE|ERROR/.test(upper)?'negative':/WAIT|ARMED|WATCH/.test(upper)?'warning':'neutral';
...
function syncProductionCommand(){
 ...
 if(!row)return;
 const score=row.score==null?'—':fmt(row.score,0),threshold=fmt(row.threshold,0),label=actionLabel(row);
 paintTile('cmdMasterValue',`${label} · ${score}/${threshold} ${row.opportunity_state||''}`.trim());
 const masterSub=$('cmdMasterSub');if(masterSub)masterSub.textContent=`Production · ${row.reason||'No executable setup'}`;
 const ready=['ENTER_LONG','ENTER_SHORT'].includes(row.action),armed=row.opportunity_state==='ARMED';
 paintTile('cmdPlanValue',ready?'PLAN READY':armed?'ARMED · WAIT':'WAITING');
 const planSub=$('cmdPlanSub');if(planSub)planSub.textContent=ready||armed?`Entry ${fmt(row.entry,8)} · SL ${fmt(row.stop_loss,8)} · TP2 ${fmt(row.tp2,8)} · R:R ${fmt(row.rr_tp2,2)}`:'No qualified Production geometry';
}
