(()=>{
  const VERSION='ATLAS_UNIFIED_TERMINAL_POLISH_V3_ANALYSIS_ONLY';
  ...
  const prettyState=s=>({
    ...
    'EXECUTION READY':'Analysis ready',
  ...
  function polishScenario(){
    ...
    if(action==='WAIT' && /READY ON TRIGGER|TRIGGER READY|SETUP READY/.test((parts.slice(2).join(' ')||'').toUpperCase())) state='Setup ready · trigger pending';
    setTextIfChanged(el,[scenario,score?`${score}%`:null,state||null].filter(Boolean).join(' · '));
  }
  function polishDecisionSemantics(){
    const action=($('auAction')?.textContent||'').trim().toUpperCase();
    const trigger=($('auTrigger')?.textContent||'').trim();
    const triggerBox=$('auTrigger');
    if(triggerBox && trigger && action==='WAIT') triggerBox.closest('.au-trigger')?.classList.add('au-trigger-pending');
    else triggerBox?.closest('.au-trigger')?.classList.remove('au-trigger-pending');
  }
  function polish(){
  ...
  style.textContent=`
    ...
    @media(max-width:820px){#atlasUnified .au-target-tile{grid-column:span 2}#atlasUnified .au-tile{min-width:0}#atlasUnified .au-tile b{font-size:14px}#atlasUnified .au-target-stack{font-size:14px}}
    @media(max-width:420px){#atlasUnified .au-plan{gap:8px}#atlasUnified .au-tile{padding:11px 10px}#atlasUnified .au-target-tile{grid-column:span 2}}
  `;
  ...
  let scheduled=false;
  const schedule=()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;polish();});};
  const start=()=>{schedule();const root=$('atlasUnified')||document.body;new MutationObserver(schedule).observe(root,{subtree:true,childList:true,characterData:true});};
