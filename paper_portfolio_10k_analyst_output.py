...
def event_id(symbol,t,g):return hashlib.sha256(f"{COHORT_LABEL}|{symbol}|{t}|{g['direction']}|{g['entry']:.12g}|{g['stop_loss']:.12g}|{g['tp2']:.12g}".encode()).hexdigest()[:24]
...
def report(m,cohort,settlements,cps,observed):
    ...
    return {'schema':SCHEMA,'cohort_label':COHORT_LABEL,'manifest_hash':m['manifest_hash'],'generated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'observed_through_at':observed.isoformat(),'canonical_contract':'analyst_output','product_horizon':'4-12H','evaluation_horizons':['4h','8h','12h'],'paper_only':True,'live_execution':False,'can_override_production':False,'production_threshold_unchanged':m['production_threshold'],'experimental_threshold':m.get('experimental_threshold'),'production_eligible':False,'methodology':'Prospective canonical analyst_output LONG/SHORT transitions only; frozen Entry/SL/TP; 4h/8h/12h checkpoints; 12h terminal evaluation. Experimental cohorts are isolated and cannot override Production.','checkpoint_summary':summary,'portfolio':{'starting_equity_usd':start,'equity_usd':round(equity,2),'net_pnl_usd':round(equity-start,2),'return_pct':round((equity/start-1)*100,4),'entries':len(cohort),'closed':len(closed),'open_or_unresolved':len(cohort)-len(closed),'wins':wins,'losses':losses,'win_rate_pct':round(100*wins/len(closed),2) if closed else None,'net_r':round(sum(rs),4),'avg_r':round(sum(rs)/len(rs),4) if rs else None,'max_drawdown_pct':round(maxdd,4)},'trades':detail}
def main():
    ...
    rep=report(m,cohort,settlements,cps,newest); COHORT.parent.mkdir(parents=True,exist_ok=True); LATEST.parent.mkdir(parents=True,exist_ok=True); INTEGRITY.parent.mkdir(parents=True,exist_ok=True); COHORT.write_text('\n'.join(canon(r) for r in cohort)+('\n' if cohort else '')); LATEST.write_text(json.dumps(rep,indent=2,sort_keys=True)); hashes=verify_append_only(cohort,prev_i); integ={'schema':'ATLAS_PAPER_10K_ANALYST_INTEGRITY_V2_ISOLATED_COHORT','cohort_label':COHORT_LABEL,'generated_at':rep['generated_at'],'manifest_hash':m['manifest_hash'],'append_only_verified':True,'row_count':len(cohort),'new_row_count':len(added),'row_hashes':hashes,'chain_sha256':hashlib.sha256(''.join(hashes).encode()).hexdigest(),'paper_only':True,'live_execution':False,'can_override_production':False,'production_eligible':False}; INTEGRITY.write_text(json.dumps(integ,indent=2,sort_keys=True)); print(json.dumps({'cohort_label':COHORT_LABEL,'added':len(added),'portfolio':rep['portfolio'],'checkpoint_summary':rep['checkpoint_summary']},indent=2))
