#!/usr/bin/env python3
"""Memory-safe Render entrypoint for ATLAS-V7.

The public Web Service should serve the UI and Production decision stack. Heavy
shadow/research evaluators are intentionally kept out of this process because
GitHub scheduled workflows already run those evaluations and persist snapshots.
This prevents archive-wide background scans from competing with the web server
for Render RAM.
"""
import os
import re
...
for name in (
    "smart_money_archive.jsonl", "smart_money_archive.json",
    "confluence_memory.jsonl", "event_memory.jsonl",
    "champion_challenger_forward.jsonl", "trade_geometry.jsonl",
    "canary_stage_state.json", "confirmed_opportunity_alerts.jsonl",
    "ai_trade_council.jsonl",
):
...
# Keep the existing Production UI patching behaviour from cloud_start.py.
...
if app_path.exists():
    ...
    if "BINANCE:HYPEUSDT" not in app_js and anchor in app_js:
        app_path.write_text(app_js.replace(anchor, anchor + hype_asset, 1), encoding="utf-8")
...
# Production-critical stack only.
...
from production_continuation_scoring import install as _install_production_continuation_scoring
_install_production_continuation_scoring(_collector)
...
from decision_engine_v7 import install as _install_decision_engine_v7
_install_decision_engine_v7(_collector)
from horizon_fit_overlay import install as _install_horizon_fit_overlay
...
from outcome_calibration_runtime import install as _install_outcome_calibration_runtime
...
# process. They are research/shadow layers and are already evaluated by scheduled
# GitHub workflows. Their cached status files remain available to the project.
